...
struct Config {
  ...
  Config()
      : port(8080), threadPoolSize(8), docRoot("./html_docs"), debug(0),
        logToConsole(0), cacheEnabled(1), cacheMaxSize(100 * 1024 * 1024),
        cacheMaxFileSize(1 * 1024 * 1024), enableTls(0), tlsCertFile(""),
        tlsKeyFile(""), tlsCipherSuites("") {}
};
...
bool parseConfigFile(const std::string &configFilePath, Config &config) {
  ...
  if (!configFile.is_open()) {
    std::cerr << "Error: Cannot open config file: " << configFilePath
              << std::endl;
    return false;
  }
  ...
  while (std::getline(configFile, line)) {
    ...
    if (key == "port") {
      try {
        ...
        std::cerr << "Warning: invalid port value in config: " << value
                  << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid thread_pool_size value in config: "
                  << value << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid debug value in config: " << value
                  << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid log_to_console value in config: "
                  << value << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid cache_enabled value in config: " << value
                  << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid cache_max_size value in config: "
                  << value << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid cache_max_file_size value in config: "
                  << value << std::endl;
      }
      ...
      try {
        ...
        std::cerr << "Warning: invalid enable_tls value in config: " << value
                  << std::endl;
      }
...
std::string resolveConfigPath(const std::string &requestedPath,
  ...
  fs::path exeDir =
      exePath.has_parent_path() ? exePath.parent_path() : fs::current_path();
 ...
 */
std::string resolvePathByConfigDir(const std::string &pathValue,
 ...
 * @brief 解析项目根目录
 *
 * 从可执行文件所在位置向上查找 CMakeLists.txt，确保无论从 build/
 * 还是项目根目录启动， 都能定位到仓库根目录。
 */
...
void printUsage(const char *programName) {
  std::cout << "Usage: " << programName << " [OPTIONS]\n"
            ...
            << "  -d, --doc-root DIR     Document root directory (default: "
               "./html_docs)\n"
 ...
 * 显示当前HTTP服务器的版本号。
 */
void printVersion() { std::cout << "CppHttpServer version 1.0.0\n"; }

void applyRuntimeConfig(HttpServer &server, const Config &config,
                        const std::string &resolvedDocRoot) {
...
int main(int argc, char *argv[]) {
   ...
   *   - '?':  遇到未知选项或缺少必需参数
   */
  while ((c = getopt_long(argc, argv, "c:p:d:t:SC:K:Y:hv", longOptions,
                          &optionIndex)) != -1) {
    switch (c) {
    case 'c':
      /** 设置配置文件路径 */
  ...
  if (logDirError) {
    std::cerr << "Failed to create log directory: " << logDir << " ("
              << logDirError.message() << ")" << std::endl;
    return 1;
  }

  std::string accessLogPath = (logDir / "access.log").string();
  std::string errorLogPath = (logDir / "error.log").string();
  if (!Logger::getInstance().init(accessLogPath, errorLogPath,
                                  debugToLogLevel(config.debug),
                                  consoleOutput)) {
  ...
  if (enableTls && (tlsCertFile.empty() || tlsKeyFile.empty())) {
    std::cerr << "Error: TLS is enabled but certificate or private key file is "
                 "missing"
              << std::endl;
    return 1;
  }
  ...
  if (enableTls) {
    if (!fs::exists(tlsCertFile)) {
      std::cerr << "Error: TLS certificate file not found: " << tlsCertFile
                << std::endl;
      return 1;
    }
    if (!fs::exists(tlsKeyFile)) {
      std::cerr << "Error: TLS private key file not found: " << tlsKeyFile
                << std::endl;
      return 1;
    }
  }
  ...
  while (server.isRunning()) {
    if (g_reloadRequested) {
      ...
      if (parseConfigFile(resolvedConfigFile, reloadedConfig)) {
        ...
        docRoot =
            resolvePathByConfigDir(reloadedConfig.docRoot, resolvedConfigFile);
        ...
        if (reloadedConfig.port != server.getPort()) {
          LOG_WARN(
              "Reloaded config changed port; restart is required to apply it");
        }
        if (reloadedConfig.threadPoolSize != server.getNumThreads()) {
          LOG_WARN("Reloaded config changed thread pool size; restart is "
        ...
        if (reloadedConfig.enableTls != previousConfig.enableTls ||
          ...
          LOG_WARN("Reloaded config changed TLS settings; restart is required "
                   "to apply it");
        }
      } else {
