...
public:
   ...
   * @brief 中间件函数类型
   *
   * 中间件可以在调用 next() 前后处理请求和响应，适合鉴权、日志、CORS
   * 和统一头处理。
   */
  using Middleware =
      std::function<void(const HttpRequest &request, HttpResponse &response,
   ...
   */
  void setTlsConfig(bool enabled, const std::string &certFile,
   ...
   * @param handler 处理函数
   */
  void registerRoutePattern(
      HttpRequest::Method method, const std::string &pathPattern,
   ...
   */
  void
  registerRoute(HttpRequest::Method method, const std::string &path,
                RequestHandler handler,
                const std::map<std::string, std::string> &requiredParams = {});
...
private:
   ...
   */
  void handleClientAccepted(int clientSocket, const std::string &clientIp,
                            int clientPort);
   ...
   * @brief 执行请求处理中间件链
   */
  void runMiddlewareChain(size_t index, const HttpRequest &request,
                          HttpResponse &response,
   ...
   * @brief 构造路由键
   */
  std::string buildRouteKey(HttpRequest::Method method,
   ...
   */
  HttpResponse
  buildUnifiedErrorResponse(const HttpRequest &request, int code,
   ...
   */
  bool handleClient(int clientSocket, const std::string &clientIp,
                    int clientPort, SSL *ssl = nullptr);
   ...
   */
  ssize_t readData(int socket, char *buffer, size_t size,
                   SSL *ssl = nullptr) const;
   ...
   */
  ssize_t sendData(int socket, const char *data, size_t size,
                   SSL *ssl = nullptr) const;
