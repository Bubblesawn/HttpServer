 ...
 * @param port 客户端端口号
 */
using TcpAcceptCallback =
    std::function<void(int fd, const std::string &ip, int port)>;
...
public:
   ...
   */
  TcpServer(const std::string &ip = "0.0.0.0", int port = 8080,
            bool useEpoll = true);
...
private:
   ...
   *
   * @return bool 成功接受或可继续重试返回 true，没有更多连接或发生错误返回
   * false
   */
